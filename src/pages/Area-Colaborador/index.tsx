...
export default function AreaColaborador() {
  ...
  const [showInatividadeAlert, setShowInatividadeAlert] = useState(false);
  const [showListaInativos, setShowListaInativos] = useState(false);
  let [diasSemCompra, setdiasSemCompra] = useState('');
  let [clientesInativosAll, setClientesInativosAll] = useState<any[]>([]);
  let [clientesInativos, setClientesInativos] = useState<any[]>([]);
  let [paginaInativos, setPaginaInativos] = useState(1);
  let [totalPaginasInativos, setTotalPaginasInativos] = useState(0);
  const qtdePaginaInativos = 5;
  const [visivelInativos, setVisivelInativos] = useState(false);
  ...
  async function VerificarClientesInativos90Dias(showAlertOnResult: boolean = true) {
    try {
      const usuarioLocal: iDadosUsuario = JSON.parse(
        localStorage.getItem('@Portal/usuario') || '{}'
      );
      ...
      await api
        ...
        .then(async () => {
          ...
          setTotalPaginasInativos(Math.ceil(result.length / qtdePaginaInativos));
          setClientesInativosAll(result);
          setClientesInativos(
            result.slice(
              (paginaInativos - 1) * qtdePaginaInativos,
              paginaInativos * qtdePaginaInativos
            ) || []
          );
          if (Array.isArray(result) && result.length > 0 && showAlertOnResult && !showListaInativos) {
  ...
  function AtualizarInativos() {
    setPaginaInativos(1);
    VerificarClientesInativos90Dias(false);
  }
  ...
  useEffect(() => {
    if (Array.isArray(clientesInativosAll) && clientesInativosAll.length > 0) {
      setClientesInativos(
        clientesInativosAll.slice(
          (paginaInativos - 1) * qtdePaginaInativos,
          paginaInativos * qtdePaginaInativos
        ) || []
      );
    }
  }, [paginaInativos, clientesInativosAll]);
  useEffect(() => {
    LoginSank();
    const interval = setInterval(LoginSank, 20000);
    return () => {
      ...
      {loading ? (
        ...
        <>
          <div className="content-global-mobile">
            ...
            <Modal
              className="modal-confirm"
              show={showInatividadeAlert}
              onHide={() => setShowInatividadeAlert(false)}
            >
              <Modal.Header closeButton>
                ...
                <div className="pedido-selec">
                  ...
                  <div style={{ marginTop: 15 }}>
                    <button
                      ...
                      className="btn btn-primary"
                      onClick={() => {
                        setShowInatividadeAlert(false);
                        setShowListaInativos(true);
                      }}
                    >
                      Sim
                    </button>
                    <button
                      style={{ width: 120 }}
                      className="btn btn-secondary"
                      onClick={() => setShowInatividadeAlert(false)}
                    >
                      Não
                    </button>
                  </div>
            ...
            <Modal
              ...
              show={showListaInativos}
              onHide={() => setShowListaInativos(false)}
              backdrop="static"
            >
              <Modal.Header closeButton>
              ...
              <Modal.Body>
                ...
                <div style={{ marginTop: 5 }} className="divbuttondrop">
                  <button
                    ...
                    onClick={() => {
                      if (!visivelInativos) {
                        setVisivelInativos(true);
                      } else {
                        setVisivelInativos(false);
                      }
                    }}
                  >
                    Filtros {visivelInativos ? <IoIosArrowUp fontSize={20} /> : <IoIosArrowDown fontSize={20} />}
                  </button>
                </div>
                {visivelInativos ? (
                  <>
                ...
                <div className="table-responsive  tabela-responsiva-pedido-realizado">
                  <div className=" table-wrap">
                    <Table responsive className="table-global table  main-table">
                      ...
                      <tbody>
                        {clientesInativos?.length > 0 ? (
                          <>
                            {clientesInativos?.map((item: any, index: number) => (
                              <tr key={index}>
                    ...
                    <Paginacao
                      total={totalPaginasInativos}
                      limit={1}
                      paginaAtual={paginaInativos}
                      setPagina={setPaginaInativos}
                    />
                  </div>
                </div>
                <button
                  style={{ width: 130, marginTop: 15 }}
                  className="btn btn-primary"
                  onClick={() => setShowListaInativos(false)}
                >
                  Voltar
                </button>
              </Modal.Body>
